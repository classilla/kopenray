...
class HTTPConnectSocketFactory implements SocketFactory {
	...
	public Socket createSocket(String host, int port, Applet applet)
		...
		return createSocket(host, port, applet.getParameter("PROXYHOST1"),
	...
	public Socket createSocket(String host, int port, String[] args)
		...
		return createSocket(host, port, readArg(args, "PROXYHOST1"), readArg(
				args, "PROXYPORT1"));
	}

	public Socket createSocket(String host, int port, String proxyHost,
			String proxyPortStr) throws IOException {
		...
		if (proxyHost == null || proxyPort == 0) {
			System.out
					.println("Incomplete parameter list for HTTPConnectSocket");
			return new Socket(host, port);
		}

		System.out.println("HTTP CONNECT via proxy " + proxyHost + " port "
				+ proxyPort);
		HTTPConnectSocket s = new HTTPConnectSocket(host, port, proxyHost,
				proxyPort);
